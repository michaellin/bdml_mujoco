...
class PandaSSLIM(ManipulatorModel):
    """
    ...
    def __init__(self, idn=0, gripper_type=None):
    ...
    @property
    def init_qpos(self):
        return np.array([0, -np.pi / 2.0, 0.00, -np.pi / 2.5 - np.pi / 2.2, 0.00, np.pi - 0.8, np.pi / 4, -0.4, 0])
